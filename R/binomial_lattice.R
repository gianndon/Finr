binomial_lattice <- function(n, S0, K, r, sigma = NA, T, optionType, exerciseType, u = NA, d = NA) {
  #' n = Anzahl Perioden
  ...
  for (i in (n - 1):0) {
    for (j in 0:i) {
      EUValue <- (1/R) * (((p * optionValues[j + 2, i + 2]) + ((1 - p) * optionValues[j + 1, i + 2])))
      if (exerciseType == "American") {
        ...
        if (intrinsicValue > EUValue) print(c(i+1,j+1))#paste("Vorz. Ausübung Lohnt sich bei Node: ",c(i+1,j+1))
